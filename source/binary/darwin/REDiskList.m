...
- (void)populateDisksBlocking {
  ...
  if (session) {
    // Register the callback to populate the disks array
    DARegisterDiskAppearedCallback(session, NULL, appendDisk, (__bridge void*)_disks);

    // Set up the run loop to process the DiskArbitration events
    CFRunLoopRef runLoop = [[NSRunLoop currentRunLoop] getCFRunLoop];
    DASessionScheduleWithRunLoop(session, runLoop, kCFRunLoopDefaultMode);

    // Run the loop for a short duration to collect disks
    CFRunLoopStop(runLoop);
    CFRunLoopRunInMode((CFStringRef)NSDefaultRunLoopMode, 0.05, NO);

    // Unregister the callback with explicit casting
    DAUnregisterCallback(session, (void *)appendDisk, (__bridge void*)_disks);
